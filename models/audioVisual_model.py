...
class AudioVisualModel(torch.nn.Module):
    ...
    def forward(self, input):
        labels = input['labels'].squeeze(1).long()  # covert back to longtensor
        vids = input['vids']
        ...
        if self.opt.log_freq:
            ...
            audio_mix_mags = F.grid_sample(audio_mix_mags, grid_warp)  # B x 1 x 512 x T => B x 1 x 256 x T
            audio_mags = F.grid_sample(audio_mags, grid_warp)  # B x 1 x 512 x T => B x 1 x 256 x T
